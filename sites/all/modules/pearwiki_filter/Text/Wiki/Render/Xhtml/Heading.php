...
class Text_Wiki_Render_Xhtml_Heading extends Text_Wiki_Render {
    ...
    function token($options)
    {
        ...
        extract($options);
        if ($text && ($level == 1 || $level == 2)) {
            $extra_anchor = preg_replace('/[^a-zA-Z0-9]+/', '_', $text);
        }

        switch($type) {
        case 'start':
            $css = $this->formatConf(' class="%s"', "css_h$level");
            return (isset($extra_anchor) ? '<h'.$level.$css.' id="'.$extra_anchor.'"' : '<h'.$level.$css.' id="'.$id.'"') .
($collapse !== null ? ' onclick="hideTOC(\''.$id.'\');"' : '').'>';
//            return (isset($extra_anchor) ? '<a name="' . $extra_anchor . '"/>':'') . '
