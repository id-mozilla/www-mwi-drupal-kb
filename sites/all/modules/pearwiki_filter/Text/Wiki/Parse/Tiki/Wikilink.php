...
class Text_Wiki_Parse_Wikilink extends Text_Wiki_Parse {
    ...
    */
    ...
    function processDescr(&$matches)
    {
        ...
        if (substr($options['text'], 0, 1) == "#") {
            $pager = strpos($options['text'], "|");
            if ($pager) {
                $opanchor = substr($options['text'], 0, $pager);
                $optext =  substr($options['text'], $pager+1);
                $options['text'] = $optext;
                $options['anchor'] = $opanchor;
            } else {
                $options['anchor'] = $options['text'];
                $options['text'] = $options['page'];
            }
        }
