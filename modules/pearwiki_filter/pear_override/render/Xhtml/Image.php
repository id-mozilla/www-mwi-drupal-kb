    ...
    */
    ...
    function token($options)
    {
        ...
        if (pearwiki_filter_use_image($pearwiki_current_format)) {
          ...
          $image = db_fetch_object(db_query("SELECT n.nid, n.title, f.filename, f.filepath FROM node n LEFT JOIN image i ON n.nid=i.nid LEFT JOIN files f ON i.fid = f.fid WHERE n.status = 1 AND n.type = 'image' AND LOWER(n.title) = LOWER('%s') LIMIT 1", $src));
          if ($image) {
            $src = file_create_url($image->filepath);
            if (!isset($options['attr']['alt'])) {
              $options['attr']['alt'] = $image->title;
            }
